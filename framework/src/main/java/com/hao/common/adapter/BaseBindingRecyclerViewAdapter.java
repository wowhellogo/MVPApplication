...
public class BaseBindingRecyclerViewAdapter<M, B extends ViewDataBinding> extends RecyclerView.Adapter<BaseBindingViewHolder<B>> {
     ...
     */
    public BaseBindingRecyclerViewAdapter(int defaultItemLayoutId) {
    ...
    @Override
    public void onBindViewHolder(BaseBindingViewHolder<B> viewHolder, int position) {
        // 在设置值的过程中忽略选中状态变化
    ...
    public BaseHeaderAndFooterAdapter getHeaderAndFooterAdapter() {
        if (mHeaderAndFooterAdapter == null) {
            synchronized (BaseBindingRecyclerViewAdapter.this) {
                if (mHeaderAndFooterAdapter == null) {
