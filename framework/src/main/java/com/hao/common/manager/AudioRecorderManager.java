...
/**
 * @Package com.daoda.aijiacommunity.common.utlis
 * @作 用:录音管理类
 * @创 建 人: linguoding
 * @日 期: 2016/4/17 0017
 */
public class AudioRecorderManager {
    ...
    public static AudioRecorderManager getInstance(Context context) {
        if (sInstance == null) {
            synchronized (AudioRecorderManager.class) {
                if (sInstance == null) {
                    sInstance = new AudioRecorderManager(context.getApplicationContext());
                }
            }
        }
        return sInstance;
    ...
    public static File getVoiceCacheDir(Context context) {
        ...
        File voiceCacheDir = context.getExternalFilesDir("voice");
